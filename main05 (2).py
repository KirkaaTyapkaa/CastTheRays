...
def angle_between_lines( line1, line2):  # gpt
...
class Grid():
    ...
    def splitWall(self,wall):

        def find_midpoint(coord1, coord2):
            midpoint_x = (coord1[0] + coord2[0]) / 2
            midpoint_y = (coord1[1] + coord2[1]) / 2
            return midpoint_x, midpoint_y

        midXY = find_midpoint(wall[0],wall[1])

        return [wall[0],midXY],[wall[1],midXY]

    def checkVisibleWalls(self,triangle, rad):  # ВОТ ЭТО основная функция вприципе этой прогрмаы
        ...
        for wall in self.walls:
           ...
           # visiblePoints = self.checkCollisionsV2(triangle[0], triangle[2], wall,visiblePoints,50, triangle )
            ...
            if len(visiblePoints) == 2: # ебать какой костыль, но да и похуй
               # visiblePoints.append(костыль)
                part1, part2 = self.splitWall(visiblePoints)
                part1.append(wall[2])
                part2.append(wall[2])
                visiblePoints.append(wall[2])
                avgDst = (self.distance(triangle[0], wall[0]) + self.distance(triangle[0], wall[1]) ) /2
                visiblePoints.append(avgDst)
                visibleWallsArr.append(visiblePoints)
               # visibleWallsArr.append(part1)
               # visibleWallsArr.append(part2)
      ...
      #  self.sortVisibleWallsAlt(triangle)
        self.drawVisibleWallsDeb()
      #  print(self.visibleWalls, len(self.visibleWalls))
        ...
        for wallInd in range(len(self.visibleWalls)):
            ...
            angle = angle_between_lines((triangle[0],triangle[1]),( triangle[0],wall[0]))
            wall[0] = (angle,dstTmp)

            dstTmp = self.distance(triangle[0], wall[1])
            angle = angle_between_lines((triangle[0], triangle[1]), ( triangle[0],wall[1]))
            wall[1] = (angle,dstTmp)


            self.visibleWalls[wallInd] = wall
     #   print(self.visibleWalls, len(self.visibleWalls), "углы растояния")
        return 2, self.visibleWalls


    def sortVisibleWallsAlt(self,triangle):
        visibleWallsArr = self.visibleWalls
        startXY = (self.plr.x, self.plr.y)
        selWallInd = 0
        for selWall in visibleWallsArr:


            pyg.draw.line(self.screen, (0, 0, 255), (self.plr.x, self.plr.y), selWall[0], 5)
            pyg.draw.line(self.screen, (0, 100, 255), (self.plr.x, self.plr.y), selWall[1], 5)

            middleXY1 = selWall[0]
            middleXY2 = selWall[1]

            angleTmp = self.angle_between_lines(((self.plr.x, self.plr.y), selWall[0]),
                                                ((self.plr.x, self.plr.y), triangle[1]))
            хуйПодзалупногоТворожка = self.plr.calcRotPos(-self.plr.fov + angleTmp)  # -self.plr.fov
            endXY1 = хуйПодзалупногоТворожка

            angleTmp = self.angle_between_lines(((self.plr.x, self.plr.y), selWall[1]),
                                                ((self.plr.x, self.plr.y), triangle[1]))
            хуйПодзалупногоТворожка = self.plr.calcRotPos(-self.plr.fov + angleTmp)  # -self.plr.fov
          ...
          #  firstFrontRay = False
            ...
            for checkWall in visibleWallsArr:  # для стен перед выбранной стеной
                #checkWall = visibleWallsArr[checkWallInd]
                firstFrontRay = False
                secondFrontRay = False
                if checkWall != selWall:
                    if self.doLinesIntersectGPT(startXY,middleXY1,checkWall[0],checkWall[1]): # проверяем левый луч
                        firstFrontRay = True
                    if self.doLinesIntersectGPT(startXY, middleXY2, checkWall[0], checkWall[1]):
                        secondFrontRay = True
                    if firstFrontRay and secondFrontRay:
                        toPop.append(checkWallInd)
                        visibleWallsArr.pop(selWallInd)
                checkWallInd += 1
           # for i in toPop
            selWallInd += 1
        self.visibleWalls =visibleWallsArr
    ...
    def sortVisibleWallsV2(self,triangle):
        ...
        for curWall in self.visibleWalls:
            ...
            for checkWall in self.visibleWalls:  # для стен перед выбранной стеной
                if checkWall != selWall :
                 #   if not (middleXY1 == checkWall[0] or middleXY1 == checkWall[1]) or not (middleXY2 == checkWall[0] or middleXY2 == checkWall[1]) :
                     #   print("yahoo")


                   # if checkWall[0] == curWall[0] or checkWall[1] == curWall
                    if self.doLinesIntersectGPT(startXY,middleXY1,checkWall[0],checkWall[1]) and not (middleXY1 == checkWall[0] or middleXY1 == checkWall[1]): # проверяем левый луч
                    ...
                    if self.doLinesIntersectGPT(startXY, middleXY2, checkWall[0], checkWall[1]) and not (middleXY2 == checkWall[0] or middleXY2 == checkWall[1]):
                        noWallsInFront = False
                        foundWalls.append(checkWall)
                        try:
                            sortedWalls.index(checkWall)
            ...
            for checkWall in self.visibleWalls:  # для стен после выбранной стеной
                if checkWall != selWall:
                    if self.doLinesIntersectGPT(middleXY1,endXY1,checkWall[0],checkWall[1]) and not (middleXY1 == checkWall[0] or middleXY1 == checkWall[1]): # проверяем левый луч
                    ...
                    if self.doLinesIntersectGPT(middleXY2,endXY2,checkWall[0],checkWall[1]) and not (middleXY2 == checkWall[0] or middleXY2 == checkWall[1]): # проверяем praвый луч
                        if checkWall in sortedWalls:
                            checkWallInd = sortedWalls.index(checkWall)
                            if checkWallInd  < selWallInd:
                                tmpWall = sortedWalls[checkWallInd]
...
class Entity():
    def __init__(self,pscreen,pgrid,renderDist):
      ...
      #  self.raysCount = rays//2
        self.forwardXY = self.calcRotPos()
      #  self.update()
    ...
    def update(self,rotArr=(False,False)): #,moveArr=(False,False,False,False),
        self.rotate(rotArr)
        #self.move(moveArr)   ІдІ нахуй
        self.forwardXY = self.calcRotPos()
        xy = (self.x, self.y)
    ...
    def line_equation(self,XY1, XY2):
        # Вычисляем коэффициент наклона (m)
        x1, y1 = XY1
        x2, y2 = XY2
        m = (y2 - y1) / (x2 - x1)
        # Вычисляем коэффициент смещения по оси y (b)
        b = y1 - m * x1
        return m, b

    def position_relative_to_line(self,XY, grad,yOffset):
        m = grad
        b = yOffset
        # Вычисляем значение y на прямой для данного x
        x, y = XY
        y_on_line = m * x + b

        # Сравниваем значение y точки с y на прямой
        if y > y_on_line:
            return True
        elif y < y_on_line:
            return False
...
class Hound(Entity):
    def __init__(self, pscreen, pgrid, renderDist,plr):
        super().__init__(pscreen,pgrid,renderDist)
        self.walkSpd = 3
        self.baseWalkSpd = self.walkSpd
        self.plr = plr
        self.x = 300
        self.y = 300
        self.fov = ma.pi /8
        self.rotSpdCoef = 20
    def update(self): #,moveArr=(False,False,False,False),
        self.forwardXY = self.calcRotPos()
        angForPlr = angle_between_lines((self.forwardXY,(self.x,self.y)),((self.plr.x,self.plr.y),(self.x,self.y)))
        #if ma.pi*2 - angForPlr < angForPlr:
           # self.angle += (ma.pi*2 - angForPlr) / 60
        print(angForPlr)


        gradTmp,yOffsetTmp = self.line_equation((self.plr.x,self.plr.y),(self.x,self.y))
        isMeHigher = self.position_relative_to_line(self.forwardXY,gradTmp,yOffsetTmp)


        if self.x > self.plr.x:
            if isMeHigher:

                self.angle += angForPlr / self.rotSpdCoef
            else:
                self.angle -= angForPlr / self.rotSpdCoef
        else:
            if isMeHigher:
        ...
        self.walkSpd = self.baseWalkSpd  * (1 - angForPlr/ma.pi )
        self.calcWalkPos(1)
        #self.rotate(rotArr)
        #self.move(moveArr)   ІдІ нахуй
        xy = (self.x, self.y)
...
class PlayScreen():
    ...
    def __init__(self,pscreen,renderDist,raysNum,plr):
        ...
        self.screen = self.bigScreen.subsurface((500,250, self.scrWidth,self.scrHeight ))
    def distance(self,point1, point2):
        return ma.sqrt((point2[0] - point1[0]) ** 2 + (point2[1] - point1[1]) ** 2)
    ...
    def update(self,wallArr,triangle):
        ...
        startX = 10 # TEMP
        dx = 0
        maxDst = self.plr.len * ma.cos(self.plr.fov)
       # maxDst = 500
        ...
        forwardRayLen =  384#ma.sqrt((self.distance(triangle[0],triangle[1])**2) - ((self.distance(triangle[1],triangle[2])/2)**2 ))
      #  print(forwardRayLen)
        ...
        for wallInd in wallArr:
          ...
          #  y1AngleCoef = ma.cos(abs(wall[0][0] - self.plr.fov)) * angleCoefMax + 1
           ...
           # if y1AngleCoef < 0 :
            ...
            trueDst1 = wall[0][1]*y1AngleCoef
            y1 = (1 -(self.xlogx(trueDst1))/(maxDst ))
            if y1 <0:
                y1 = 0
            y1 = y1 * maxHeight
            y1 = self.calcWallHeight(trueDst1, self.maxDst)
         ...
         #   y2AngleCoef = (1 - abs(wall[1][0] - self.plr.fov) / self.plr.fov) * angleCoefMax + 1
          #  y2AngleCoef = ma.cos(abs(wall[1][0] - self.plr.fov)) * angleCoefMax + 1
            ...
            trueDst2 = wall[1][1] * y2AngleCoef
            y2 = (1- (self.xlogx(wall[1][1]*y2AngleCoef)) / (maxDst) )

            if y2 <0:
                y2 = 0
            y2 = y2 * maxHeight
            y2 = self.calcWallHeight(trueDst2, self.maxDst)
            ...
            avgDst = (trueDst1 +trueDst2 )/2
            colDstCoef = 1 - avgDst/maxDst
            if wall[3] > maxDst:
                colDstCoef = 0
            else:
                colDstCoef = 1 - wall[3]/maxDst
            col = (255*colDebCoef*0,255*colDebCoef,255*colDebCoef*0)
            col = (wall[2][0] * colDstCoef,wall[2][1] * colDstCoef,wall[2][2] * colDstCoef )
            pyg.draw.polygon(self.screen,col,((x2,y2+middle),(x2,middle-y2),(x1,middle-y1),(x1,middle+y1)))



    def calcWallHeight(self,x,maxDst):
        slope = 0.1
        return maxDst*10/x
        return slope * ((1/(0.1*x)) - x + maxDst )

    def xlogx(self,x):
        return  x
        return x + 20*ma.log(x-3,ma.e) - 10**2
...
def main():
    ...
    testHound = Hound(screen,grid,renderDst,plr)
    grid.plr = plr
  ...
  #  print(1/0.0000000050)
   ...
   # grid.addWall((320, 100), (600, 50),(200,255,0))

   # grid.addWall((150, 50), (450, 550), (100, 255, 90))
   # grid.addWall((250, 150), (300, 200), (255, 0, 50))


    grid.addWall((100,100),(300,150),(50,150,20))
    grid.addWall((300, 150),(600, 100) , (50, 150, 20))

    grid.addWall((100, 400), (300, 350), (50, 150, 20))
    grid.addWall((300, 350), (600, 400), (50, 150, 20))

    grid.addWall((300, 220), (300, 280), (150, 150, 20))

    grid.addWall((300, 220), (350, 220), (150, 150, 20))
    grid.addWall((300, 280), (350, 280), (150, 150, 20))

    grid.addWall((350, 220), (350, 280), (150, 150, 20))

    grid.addWall((100, 100), (200, 300), (50, 150, 20))
    grid.addWall((100, 400), (200, 300), (50, 150, 20))
   # grid.addWall((300, 150), (600, 100), (50, 150, 20))
#    debRect =
    while True:
       ...
       # plr.rotate(cameraDir)
        ...
        visionTriangle = plr.update(cameraDir)
        testHound.update()
        inWall, visbleWalls = grid.checkVisibleWalls(visionTriangle,plr.rad)
        plr.move(moveDir,inWall)
        #print(wallArr)
